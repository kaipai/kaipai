...
class ImageService implements ServiceManagerAwareInterface{//类定义开始
    ...
    public function getImageHander($img) {
        $size = @getimagesize($img);
        switch ($size['mime']) {
            case 'image/jpeg': return imagecreatefromjpeg($img);
            case 'image/gif' : return imagecreatefromgif($img);
            case 'image/png' : return imagecreatefrompng($img);
     ...
     * @return void
      +----------------------------------------------------------
     */
    public function showImg($imgFile, $text = '', $width = 80, $height = 30) {
        //获取图像文件信息
     ...
     * @return void
      +----------------------------------------------------------
     */
    public function thumb($image, $thumbname, $type = '', $width = 200, $height = 50, $interlace = true) {
        // 获取原图信息
     ...
     * @return string
      +----------------------------------------------------------
     */
    public function buildString($string, $size = '', $font = '', $rgb = array(), $filename = '', $type = 'png', $disturb = 1, $border = true) {
        if (is_string($size))
            $size = explode(',', $size);
        $width = $size[0];
        $height = $size[1];
     ...
     * @return string
      +----------------------------------------------------------
     */
    public function buildImageVerify($length = 4, $mode = 1, $type = 'gif', $width = 48, $height = 22, $verifyName = 'verify') {
        $randval = $this->sm->get('COM\Service\StringService')->rand_string($length, $mode);
...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
    ...
    public function GBVerify($length = 4, $type = 'png', $width = 180, $height = 50, $fontface = 'simhei.ttf', $verifyName = 'verify') {
        $code = $this->sm->get('COM\Service\StringService')->rand_string($length, 4);
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $codex = $this->sm->get('COM\Service\StringService')->msubstr($code, $i, 1);
     ...
     * @return string
      +----------------------------------------------------------
     */
    public function showAdvVerify($type = 'png', $width = 180, $height = 40, $verifyName = 'verifyCode') {
        $rand = range('a', 'z');
        shuffle($rand);
