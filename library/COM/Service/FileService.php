...
class FileService
{
    private $targetAbs; // 指定上传文件存放在服务器的绝对路径
    private $targetRel; // 指定上传文件存放在服务器的相对路径
    private $ifThumb = 0; // 是否需要生成缩略图
    private $thumbW = 100; // 缩略图宽
    private $thumbH = 100; // 缩略图高
    private $ifWaterMark; // 是否加水印
    ...
    public function upload(Request $request)
    {
        ...
        foreach ($files as $k => $v) {
            ...
            $filters[] = new \Zend\Filter\File\Rename(
                array(
                    'target' => $this->targetAbs . md5($tmpName) . $ext,
                    "randomize" => true
                )
            );
        ...
        foreach ($files as $k => $v) {
            ...
            $data = array(
                ...
                'path' => $this->targetRel . $fileInfo[$k]['name'],
                'type' => $fileInfo[$k]['type']
            );
    ...
    {
        $this->targetAbs = $_SERVER['DOCUMENT_ROOT'] . '/uploads/' . date('Ymd') . '/';
        ...
        if (!is_dir($this->targetAbs . $targetDir)) {
            ...
            $this->targetAbs .= $targetDir;
            $this->targetRel .= $targetDir;
     ...
     * @param int [0-100] options['transparency']
     * 设置水印属性
     */
    public function setWaterMark($options)
    {
        ...
        if (isset($options['text'])) {
            $this->waterText = $options['text'];
        }
        if (isset($options['transparency'])) {
            $this->waterPct = $options['transparency'];
        }
     ...
     *
     * 更改上传文件类型 ...
     * @param $str 'jpg|gif|jpeg|png|bmp' or 'xls|xlsx';
     */
    public function changeAllowType($str)
    {
        $this->allowType = $str;
        return $this;
    }
    ...
    {
        return in_array(ltrim(strtolower($ext), '.'), explode('|', $this->allowType));
    }

    /**
     * 删除图片
     */
    public function unlink($dstFile)
    {
        $dstFile = $_SERVER['DOCUMENT_ROOT'] . $dstFile;
     ...
     * @return boolean
     */
    public function makeWatermark($source, $waterPos = 0, $waterImg = '', $waterText = '', $attribute = '', $waterPct = 50, $waterQuality = 75, $dstsrc = null)
    {
