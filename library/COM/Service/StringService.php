...
// | 字符串处理类
// +----------------------------------------------------------------------
// | Copyright (c) 2011 http://www.chemayi.com All rights reserved.
// +----------------------------------------------------------------------
// | Author: Pipes
// +----------------------------------------------------------------------
...
class StringService {
     ...
     * @modify Fastlane 2013-9-2 10:09 修改最后一个参数$suffix默认值为'...'；修复了不能使用自定义符号显示截断字符（串）；将该类格式化
    +----------------------------------------------------------
     * @return string
    +----------------------------------------------------------
     */
    public function msubstr($str, $start = 0, $length, $charset = "utf-8", $suffix = '...') {
        if (!strlen($str)) {
            return '';
        }
    ...
    public function build_format_rand($format, $number = 1) {
        ...
        for ($j = 0; $j < $number; $j++) {
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                switch ($char) {
                    case "*"://字母和数字混合
                        $strtemp .= $this->rand_string(1);
                        break;
                    case "#"://数字
                        $strtemp .= $this->rand_string(1, 1);
                        break;
                    case "$"://大写字母
                        $strtemp .= $this->rand_string(1, 2);
                        break;
                    default://其他格式均不转换
                        $strtemp .= $char;
                        break;
