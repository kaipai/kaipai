                ...
                /*if(!empty($v['keepPrice']) && $v['keepPrice'] > $v['currPrice']){
                    ...
                    if(empty($v['auctionMemberID'])){
                        if(!empty($v['specialID'])){
                            ...
                            $this->productModel->update(array('auctionStatus' => 0, 'isPaid' => 0, 'startTime' => new Expression('null'), 'endTime' => new Expression('null'), 'soldStatus' => 1, 'unSoldTime' => time(), 'publishAtOnce' => 0), array('productID' => $v['productID']));
                        }
                        $this->memberProductInterestModel->delete(array('productID' => $v['productID']));
